 ...
 *  の責任を負わない．
 * 
 *  $Id: test_sem2.c 310 2015-02-08 13:46:46Z ertl-hiro $
 */
 ...
 *	22:	act_tsk(TASK2)
 *	== TASK2（優先度：高）==
 *	23:	tslp_tsk(10000U) -> E_TMOUT
 *	== TASK1（続き）==
 *	24:	wai_sem(SEM1)
 *	== TASK3（続き）==
 *	25:	ext_tsk() -> noreturn
 *	== TASK2（続き）==
 *	26:	sta_alm(ALM1, 10000U)
 *		call(wait_var())
 *	== ALM1-5 ==
 *	27:	sig_sem(SEM1)						... (B-5)
 *		call(signal_var())
 ...
 *	28:	ext_tsk() -> noreturn
 *	== TASK1（続き）==
 *	29:	sta_alm(ALM1, 10000U)
 *	30:	twai_sem(SEM1, 100000U) -> E_RLWAI
 *	== ALM1-6 ==
 *	31:	rel_wai(TASK1)
 *		RETURN
 *	== TASK1（続き）==
 *	32:	sta_alm(ALM1, 100000U)
 *	33:	twai_sem(SEM1, 10000U) -> E_TMOUT		... (H)
 *	34:	stp_alm(ALM1)
 *	35:	END
 */
...
task1(intptr_t exinf)
{
	...
	check_point(30);
	ercd = twai_sem(SEM1, 100000U);
	check_ercd(ercd, E_RLWAI);

	check_point(32);
	ercd = sta_alm(ALM1, 100000U);
	check_ercd(ercd, E_OK);
