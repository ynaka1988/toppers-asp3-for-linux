...
#  の責任を負わない．
# 
#  $Id: gentest.rb 578 2016-02-03 07:23:53Z ertl-hiro $
# 
...
class PUCode
  ...
  def generateCode
    ...
    @variableList.keys.each do |varName|
      print("\t#{@variableList[varName]}")
      print(@variableList[varName].length < 4 ? "\t\t" : "\t")
      print("#{varName};\n")
    end
    if @silFlag
      print("\tSIL_PRE_LOC;\n")
...
def genServiceCall(pu, svc_call, error_code)
  ...
  if /^([a-z_]+)\((.*)\)$/ =~ svc_call
    ...
    if !$parameterPosition[svcName].nil? \
					&& params.size >= $parameterPosition[svcName]
      varName = params[$parameterPosition[svcName] - 1].sub(/^\&/, "")
      typeName = $parameterType[svcName]
      pu.addVariable(varName, typeName)
    end
  end
